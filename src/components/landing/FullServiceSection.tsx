...
export const FullServiceSection = () => {
  ...
  return <section ref={sectionRef} id="funktionen" className="py-20 container mx-auto px-4">
      <div className="text-center mb-16">
        ...
        <p className="text-sm md:text-base text-muted-foreground max-w-3xl mx-auto font-medium">Ihr Vorteil: Sie gewinnen Freiraum für Ihr Kerngeschäft, während wir für das professionelle Setup, konstantes Monitoring und die garantierte Leistungssteigerung sorgen.</p>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8">
        {services.map((service, index) => <div key={index} className={`p-6 rounded-lg border border-border bg-card hover:shadow-lg transition-all duration-500 md:hover:scale-105 ${isVisible ? 'animate-fade-in opacity-100' : 'opacity-0'}`} style={{
